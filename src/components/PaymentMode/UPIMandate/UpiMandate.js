...
import PayByUpiMandate from "./PayByUPIMandate";
...
class UpiMandate extends Component {
    state = {}
    ...
    upiPayNowSuccessCb = (response) => {
        document.getElementById("UPIMandateForm").submit();
    ...
    render() {
        ...
        merchant_payment_data.payment_mode_data.map(paymentModeConfigData => {
            if (paymentModeConfigData.payment_mode_id == 15) {
        ...
        let renderPaymentOptionByUserChoise = <PayByUpiMandate upiSuccessCb={this.upiPayNowSuccessCb} upiFailureCb={this.upiPayNowFailureCb} />
        return (
            <>
                <div
                    id="collapseupimandate"
                    className="card-body collapse active show"
                    data-parent="#accordion"
                >
                    <h2>Select your UPI app</h2>
                    <div class="col-12 upibhim">
                        <form id="UPIMandateForm" method="post" action={process.env.REACT_APP_PG_CONTROLLER + "pinepg/v2/submit/upimandate"}  >
...
export default connect(mapStateToProps, { setSelectUpiOption, payUpiGPay })(UpiMandate);
